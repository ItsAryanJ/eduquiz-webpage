...
$stmt->store_result();
if ($stmt->num_rows > 0) {
    echo "<script>alert('ID already exists.'); window.location.href = 'admin_dashboard.html';</script>";
    exit();  
}
$stmt->close();
...
if ($stmt->execute()) {
    echo "<script>alert('Registration successful! You will be redirected to the admin dashboard.'); window.location.href = 'admin_dashboard.html';</script>";  
    exit();
} else {
